...
export class CertificateService {
   ...
   */
  async generateCertificateClaims(userId: string, tenantId: string = 'default-tenant'): Promise<DestructionCertificateClaims> {
    logger.info({ userId, tenantId }, 'Generating destruction certificate claims');
    ...
    // Aggregate lineage summary from BigQuery
    const lineageData = await this.lineageRepo.getUserLineage(userId, tenantId);
    const lineageDestinations = lineageData.destinations.length > 0
      ? lineageData.destinations
      : [...new Set(keyStatus.destinations || [])].map((name) => ({
          name,
          lastSeen: new Date(keyStatus.shred_at || keyStatus.shredAt || keyStatus.created_at || keyStatus.createdAt || new Date().toISOString()),
        }));

    const lineageSummary = lineageDestinations.map(dest => ({
      system: dest.name,
      status: 'ERASED' as const,
      timestamp: dest.lastSeen instanceof Date ? dest.lastSeen.toISOString() : (dest.lastSeen as string)
    }));
  ...
  async issueAndStoreCertificate(userId: string, deletionRequestId: string, tenantId: string = 'default-tenant'): Promise<{ certificate: string; gcsPath: string }> {
    const claims = await this.generateCertificateClaims(userId, tenantId);
