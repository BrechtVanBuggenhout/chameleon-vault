...
import { validateRequest, createAnalystClaimsSchema, claimAnalystTokenSchema } from '../middleware/validation.js';
  ...
  fastify.post<{ Params: { token: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        logger.info({ correlationId: context.correlationId, analystEmail: result.analystEmail }, 'Analyst credential claimed');
        return reply.status(200).send({ apiKey: result.apiKey, analystEmail: result.analystEmail });
      } catch (error: unknown) {
        const err = error as { statusCode?: number; message?: string; errors?: unknown };
        if (err.statusCode === 400) {
          return reply.status(400).send({ error: err.message || 'Validation Error', errors: err.errors, statusCode: 400 });
